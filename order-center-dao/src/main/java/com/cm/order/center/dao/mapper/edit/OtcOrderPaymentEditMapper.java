/**
* @Title: OtcOrderPaymentEditMapper
* @Package com.cm.order.center.dao.mapper.edit
* @Description: 订单支付表--增删改接口类
* @author chenmin
* @date Thu Aug 18 09:09:05 CST 2022
* @version V1.0
*/
package com.cm.order.center.dao.mapper.edit;

import com.cm.architecture.jdbc.mapper.PersistenceEditMapper;
import com.cm.order.center.dao.po.OtcOrderPaymentPo;
/**
* @ClassName: OtcOrderPaymentEditMapper
* @Description: 订单支付表--增删改接口类
* @author chenmin
* @date Thu Aug 18 09:09:05 CST 2022
 */
public interface OtcOrderPaymentEditMapper extends PersistenceEditMapper<OtcOrderPaymentPo>{

}
